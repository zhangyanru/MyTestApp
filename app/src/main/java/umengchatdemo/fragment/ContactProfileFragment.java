...
import umengchatdemo.interfa.IParent;
...
public  class ContactProfileFragment extends Fragment implements OnClickListener {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		mUserId = UMUtils.getInstance().getmIMKit().getIMCore().getLoginUserId();
	...
	private void addContactCacheUpdateListener() {
		if(mContactCacheUpdateListener!=null&& UMUtils.getInstance().getmIMKit()!=null)
			UMUtils.getInstance().getmIMKit().getContactService().addContactCacheUpdateListener(mContactCacheUpdateListener);
	}
	private void removeContactCacheUpdateListener() {
		if(mContactCacheUpdateListener!=null&& UMUtils.getInstance().getmIMKit()!=null)
			UMUtils.getInstance().getmIMKit().getContactService().removeContactCacheUpdateListener(mContactCacheUpdateListener);
	}
...
//		titleBar.setBackgroundColor(Color.parseColor("#00b4ff"));
	...
	private IYWContactService getContactService(){
		IYWContactService contactService = UMUtils.getInstance().getmIMKit().getContactService();
		return contactService;
	}
	private YWIMKit getIMkit(){
		YWIMKit imkit = UMUtils.getInstance().getmIMKit();
		return imkit;
	}
...
//			}
	...
	private void sendMessage(YWProfileInfo mYWProfileInfo) {
		if(mYWProfileInfo.userId.equals(UMUtils.getInstance().getmIMKit().getIMCore().getLoginUserId())) {
